/* ==========================================================================
 * HyicoPlatform 单机详情逻辑：姿态、地图、实时数据、告警、历史、只读控制区。
 * ========================================================================== */
(function () {
    'use strict';
    ...
    const esc = function (v) { return window.HYICO.escapeHtml(v); };
    const i18n = () => window.HYICO.i18n;
    ...
    function renderHeader() {
        ...
        document.getElementById('droneMeta').textContent =
            ...
            (d.online && d.mode ? ' · ' + window.HYICO.modeLabel(d.mode) : '');
    ...
    function renderData(d) {
        ...
        html += kvRow(t.mode, esc(window.HYICO.modeLabel(d.mode)));
        html += kvRow(t.systemStatus, esc(d.system_status || '--'));
        html += kvRow(t.battery, batt == null ? '--' : batt + '%', battCls);
        html += kvRow(t.voltage, window.HYICO.fmtVolt(d.voltage));
        html += kvRow(t.current, window.HYICO.fmtCurr(d.current));
        html += kvRow(t.alt, window.HYICO.fmtAlt(d.alt));
        html += kvRow(t.relAlt, window.HYICO.fmtAlt(d.rel_alt));
        html += kvRow(t.heading, window.HYICO.fmt(d.heading, 0) + '°', 'kv__val--mono');
        html += kvRow(t.groundspeed, window.HYICO.fmtSpeed(d.groundspeed));
        html += kvRow(t.airspeed, window.HYICO.fmtSpeed(d.airspeed));
        html += kvRow(t.climb, window.HYICO.fmt(d.climb, 1) + ' m/s');
        html += kvRow(t.throttle, window.HYICO.fmtInt(d.throttle) + '%');
        html += kvRow(t.satellites, window.HYICO.fmtInt(d.satellites));
        ...
        html += kvRow(t.localPos,
            (d.local_x == null) ? '--'
                : 'X ' + window.HYICO.fmt(d.local_x, 2) + ' / Y ' + window.HYICO.fmt(d.local_y, 2) + ' / Z ' + window.HYICO.fmt(d.local_z, 2) + ' m',
            'kv__val--mono');
        html += kvRow(t.vibration,
            (d.vibration_x == null) ? '--'
                : 'X ' + window.HYICO.fmt(d.vibration_x, 2) + ' / Y ' + window.HYICO.fmt(d.vibration_y, 2) + ' / Z ' + window.HYICO.fmt(d.vibration_z, 2),
            'kv__val--mono');
        html += kvRow(t.range, d.range_m == null ? '--' : window.HYICO.fmt(d.range_m, 2) + ' m');
        html += kvRow(t.mcuTemp, d.mcu_temp == null ? '--' : window.HYICO.fmt(d.mcu_temp, 1) + ' °C', d.mcu_temp != null && d.mcu_temp > 75 ? 'kv__val--warn' : '');
        html += kvRow(t.mcuVoltage, d.mcu_voltage == null ? '--' : window.HYICO.fmt(d.mcu_voltage, 2) + ' V');
        html += kvRow(t.rssi, d.rssi == null ? '--' : window.HYICO.fmtInt(d.rssi) + ' dBm', d.rssi != null && d.rssi < -90 ? 'kv__val--bad' : '');
        html += kvRow(t.load, d.load == null ? '--' : window.HYICO.fmt(d.load, 1) + ' %');
        html += kvRow(t.windSpeed, window.HYICO.fmtSpeed(d.wind_speed));
        html += kvRow(t.ekf, d.ekf_ok
            ? '<span class="kv__val--ok">' + esc(t.ok) + '</span>'
            : (d.ekf_ok == null ? '--' : '<span class="kv__val--bad">' + esc(t.bad) + '</span>'));
        html += kvRow(t.groundDistance, d.ground_distance == null ? '--' : window.HYICO.fmt(d.ground_distance, 2) + ' m');
        html += kvRow(t.lastSeen, window.HYICO.relTime(d.last_seen));
        ...
        box.innerHTML = sorted.map(function (a) {
            ...
            return '<div class="alert-item ' + cls + '">' +
                '<div class="alert-item__body">' +
                    '<div class="alert-item__title">' + esc(window.HYICO.alertRuleLabel(a.rule)) + '</div>' +
                    '<div class="alert-item__msg">' + esc(a.message) + '</div>' +
                    '<div class="alert-item__time">' + (ts ? window.HYICO.fmtTime(ts) : '刚刚') + '</div>' +
                '</div>' +
            '</div>';
        }).join('');
    }
    ...
    function renderCommands() {
        ...
        window.HYICO.api.health().then(function (h) {
            ...
            host.querySelectorAll('.command-btn').forEach(function (btn) {
                ...
                btn.addEventListener('click', function () {
                    ...
                    window.HYICO.api.command(SYSID, btn.dataset.cmd, {})
                        .then(function () { window.HYICO.toast('指令已发送', 'success'); })
                        .catch(function (e) { window.HYICO.toast(e.message || '指令失败', 'error'); });
                });
            });
        }).catch(function () { /* 保持只读 */ });
    }
    ...
    function renderHistory(history) {
        ...
        body.innerHTML = rows.map(function (r) {
            ...
            return '<tr>' +
                '<td>' + window.HYICO.fmtTime(r.ts) + '</td>' +
                '<td>' + batt + '</td>' +
                '<td>' + window.HYICO.fmtAlt(r.alt) + '</td>' +
                '<td>' + esc(window.HYICO.modeLabel(r.mode)) + '</td>' +
                '<td>' + (r.armed ? '✔ 已解锁' : '—') + '</td>' +
            '</tr>';
        }).join('');
    }
    ...
    async function init() {
        ...
        await window.HYICO.loadI18n('drone.json');
        window.HYICO.initTheme();
        window.HYICO.renderNav('drone');
        document.getElementById('backLink').textContent = '← ' + i18n().back;

        try {
            current = await window.HYICO.api.drone(SYSID);
        } catch (err) {
            if (err.status === 404) {
            ...
            window.HYICO.toast(i18n().errors.loadFailed, 'error');
            return;
        }
        ...
        window.HYICO.api.alerts(SYSID, true).then(function (data) {
        ...
        window.HYICO.api.history(SYSID, 300).then(function (data) {
