/* ==========================================================================
 * HyicoPlatform 机队总览逻辑：REST 首屏 + WebSocket 实时刷新。
 * ========================================================================== */
(function () {
    'use strict';
    ...
    function renderStats() {
        ...
        bar.innerHTML =
            '<div class="stat stat--ok"><div class="stat__value">' + online + '</div><div class="stat__label">' +
                window.HYICO.escapeHtml(i18n.stats.online) + '</div></div>' +
            '<div class="stat"><div class="stat__value">' + known + '</div><div class="stat__label">' +
                window.HYICO.escapeHtml(i18n.stats.known) + '</div></div>' +
            '<div class="stat ' + (alerting ? 'stat--danger' : 'stat--ok') + '"><div class="stat__value">' + alerting +
                '</div><div class="stat__label">' + window.HYICO.escapeHtml(i18n.stats.alerting) + '</div></div>';
    }
    ...
    function renderCard(d) {
        const i18n = window.HYICO.i18n;
        const esc = window.HYICO.escapeHtml;
        ...
        const altTxt = d.rel_alt != null ? window.HYICO.fmtAlt(d.rel_alt) : window.HYICO.fmtAlt(d.alt);
        const modeTxt = window.HYICO.modeLabel(d.mode);
        ...
        return '<article class="drone-card' + (maxLevel ? ' is-alert' : '') + (d.online ? '' : ' is-offline') + '">' +
            ...
            '<div class="drone-card__foot">' +
                '<span class="drone-card__last">' + esc(i18n.card.lastSeen) + '：' +
                    window.HYICO.relTime(d.last_seen) + '</span>' +
    ...
    function renderGrid() {
        ...
        if (!drones.length) {
            grid.innerHTML =
                ...
                '<div class="empty__title">' + window.HYICO.escapeHtml(i18n.empty.title) + '</div>' +
                '<p class="empty__desc">' + window.HYICO.escapeHtml(i18n.empty.desc) + '</p></div>';
            return;
        }
    ...
    async function loadInitial() {
        ...
        try {
            const data = await window.HYICO.api.drones();
            drones = data.drones || [];
            ...
            if (err.status !== 401) {
                grid.innerHTML = '<div class="empty"><div class="empty__icon">⚠️</div>' +
                    ...
                    '<p class="empty__desc">' + window.HYICO.escapeHtml(window.HYICO.i18n.errors.loadFailed) + '</p></div>';
            }
        }
    }

    document.addEventListener('DOMContentLoaded', async function () {
        await window.HYICO.loadI18n('dashboard.json');
        window.HYICO.initTheme();
        window.HYICO.renderNav('dashboard');

        document.getElementById('pageTitle').textContent = window.HYICO.i18n.title;
        document.getElementById('pageSubtitle').textContent = window.HYICO.i18n.subtitle;

        document.getElementById('refreshBtn').addEventListener('click', function () {
            window.HYICO.api.drones().then(function (data) {
                drones = data.drones || [];
                render();
